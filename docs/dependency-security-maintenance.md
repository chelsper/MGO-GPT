...
This document tracks the dependency assessment and its isolated remediation
batches. It was prepared on August 8, 2026 while production was serving commit
`43fa832`.

## Completed Router and Server Batch

The first remediation batch updates only the routing and HTTP-server packages:

- React Router packages to `7.18.2`
- Hono to `4.13.1`
- `react-router-hono-server` to `2.26.0`

No application source, authentication, Blackbaud, database, or UI files were
changed. The package lockfile was regenerated by npm.

Validation completed on Node `22.20.0`, the minimum runtime required by
`react-router-hono-server@2.26.0` and below the live Vercel Node 24 runtime:

- 27 Vitest files and 156 tests passed.
- The production React Router build passed.
- `npm run check:release` passed.
- `npm audit --omit=dev` reduced from 24 findings to 22, removing the direct
  React Router and Hono findings addressed by this batch.

The remaining `@hono/node-server` advisory is reached through
`react-router-hono-server`; its available automated fix requires the separate,
breaking `react-router-hono-server@3` migration. It is intentionally deferred.
