...
- Phone comparison ignores punctuation and an optional North American +1 country code, but preserves extensions and distinct numbers. NXT reformatting no longer creates a false primary-setting hold or duplicate phone.
- **Verify in NXT and finish** is available on held standard-import rows with a known target and saved write history. It reads the saved identity and every requested detail afresh, including contact types, primary settings, provided address fields, constituency dates, and education details. Only a complete match marks the app row Applied and advances review; it sends no NXT writes. Missing, ambiguous, incomplete, or unsupported results stay held with specific verification results and can be checked again. Active sends and unknown constituent-creation outcomes cannot use this action.
- The original send results are preserved, with a separate completion audit recording the reviewer, prior hold, and zero NXT writes. Saving requires the row's status, target, plan, preview, and audit to remain unchanged. Pre-send forms are hidden after an attempted send so old snapshots are not presented as current NXT data. Verified counts require every planned change to be confirmed, not merely a check timestamp.

For an uncertain write, open the saved row and compare its results with NXT before doing anything else. If the saved record already contains every requested detail, use **Verify in NXT and finish** instead of sending again. Do not upload the whole row again as a shortcut. Prepare a separate import containing only confirmed outstanding changes after resolving what was saved. Constituent-code replacement also holds if target creation is uncertain; it does not then attempt an automatic restoration. A confirmed restoration creates a new code-row ID and requires a fresh review rather than replaying the original replacement plan.
