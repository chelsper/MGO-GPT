...
- The portfolio endpoint projects only dates and original check times from the existing `prospect-activity-v1` cache and the new authorized workspace activity snapshots, alongside its existing bulk contact query. These entries come from the latest-gift endpoint and complete action responses; they are not inferred from FY gift totals, narrative text, scheduled actions or maintenance timestamps.
...
- Each constituent has independent gift/action rows in `portfolio_activity_snapshots`. These hold record ID, date, original check time, service-user provenance and queue state, not action notes or full donor records. Reuse checks only the service user's exact existing activity-cache key. Other saved narrative/FY payloads cannot establish an all-time latest date and are not promoted.
