...
it("follows query continuation tokens without changing the query definition", async () => {
  ...
  expect(await executeBlackbaudListQuery({ ...options, maxPages: 2, dataModelName: "renxt-action", definition: { output: { items: [{ field_id: "category" }] } } })).toEqual([{ id: "1" }, { id: "2" }]);
  expect(JSON.parse(fetch.mock.calls[1][1].body)).toMatchObject({ continuation_token: "next-page", definition: { output: { items: [{ field_id: "category" }] } } });
});
