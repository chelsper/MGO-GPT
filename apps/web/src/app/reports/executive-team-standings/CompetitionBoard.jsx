...
import { comparisonChange, periodText, rankStandings, RANKING_MODES, scoreText } from "./standingsPresentation";
...
export default function CompetitionBoard({ entries, mode, onModeChange, fiscalYear, comparison }) {
  const [compare, setCompare] = useState(true);
  const comparing = Boolean(comparison && compare);
  const ranked = rankStandings(entries, mode);
  ...
  return (
    <section aria-label="Team leaderboard" className="standings-competition">
      <div className="standings-leaders">
        <LeaderCard entries={entries} mode="raised" fiscalYear={`${fiscalYear}${comparison ? " YTD" : ""}`} />
        <LeaderCard entries={entries} mode="actions" fiscalYear={`${fiscalYear}${comparison ? " YTD" : ""}`} />
      </div>
      <div className="standings-board">
        <div className="standings-board-heading">
          <div><p className="standings-eyebrow">Season standings</p><h2>{fiscalYear}{comparison ? " to date" : ""} leaderboard</h2></div>
        ...
        {comparison ? <div className="standings-period-controls">
          <label><input type="checkbox" checked={compare} onChange={(event) => setCompare(event.target.checked)} /> Compare with last FY to date</label>
          <p><strong>{comparison.current.label} YTD:</strong> {periodText(comparison.current)}<br /><strong>{comparison.prior.label} YTD:</strong> {periodText(comparison.prior)}</p>
          <span>Cutoffs are fixed to this saved snapshot in Eastern time. Both years use the same month/day, with February 29 clamped to February 28 when needed.</span>
        </div> : <p className="standings-upgrade" role="status">Year-to-date comparison: Refresh required. Existing scores below retain their original full-fiscal-year definition until a complete comparison is saved.</p>}
        ...
        <div className="standings-table-scroll" tabIndex={0} role="region" aria-label="Ranked standings table">
          <table className="standings-table">
            ...
            <thead><tr><th scope="col">Rank</th><th scope="col">MGO</th>{comparing ? <><th scope="col" aria-sort="descending">{comparison.current.label} YTD</th><th scope="col" className="standings-comparison-extra">{comparison.prior.label} YTD</th><th scope="col" className="standings-comparison-extra">Change</th></> : scoreColumns.map((column) => <th key={column} scope="col" className={mode === column ? "" : "standings-secondary-score"} aria-sort={mode === column ? "descending" : "none"}>{column === "raised" ? `${fiscalYear}${comparison ? " YTD" : ""} raised` : "High-value actions"}</th>)}</tr></thead>
            <tbody>{ranked.map((entry) => {
              ...
              return (
                <tr key={entry.userId} className={leading ? "standings-leading" : ""}>
                  ...
                  {comparing ? <><td className="standings-selected-score">{scoreText(entry.score, mode)}<span className="standings-mobile-comparison"><span>{comparison.prior.label} YTD: {scoreText(entry.priorYearToDate?.[mode === "raised" ? "raised" : "highValueActions"], mode)}</span><span>{comparisonChange(entry.score, entry.priorYearToDate?.[mode === "raised" ? "raised" : "highValueActions"], mode)}</span></span></td><td className="standings-comparison-extra">{scoreText(entry.priorYearToDate?.[mode === "raised" ? "raised" : "highValueActions"], mode)}</td><td className="standings-comparison-extra">{comparisonChange(entry.score, entry.priorYearToDate?.[mode === "raised" ? "raised" : "highValueActions"], mode)}</td></> : scoreColumns.map((column) => <td key={column} className={mode === column ? "standings-selected-score" : "standings-secondary-score"}>{column === "actions" && entry.highValueActionsThisFiscalYear === undefined ? "Refresh required" : scoreText(entry[RANKING_MODES[column].field], column)}</td>)}
                </tr>
              );
            })}</tbody>
          </table>
      ...
      <details className="standings-rules">
        ...
        <dl>
          <dt>Fiscal year</dt><dd>July 1 through June 30. New snapshots rank through the displayed as-of date, excluding future-dated gifts and actions. Prior-year comparison uses the matching cutoff, not the full prior fiscal year. The roster is the current active MGO team, not a reconstructed prior-year roster.</dd>
