...
export default function QueryResultsTable({
  ...
  if (!isValidDashboardTableData({ headers, rows }))
    return <p className={styles.unknown}>Not refreshed</p>;

  const settings = new Map(
    columnSettings.map((column) => [column.header, column]),
  );
  ...
  return (
    <div className={styles.queryResults}>
      <p id={hintId} className={compact ? listStyles.srOnly : styles.help}>
        Scroll across to see all columns. Select a column heading to sort.
      </p>
      <div
      ...
      >
        <table
          className={`${styles.table} ${styles.queryTable}`}
          aria-label={title}
        >
          <thead>
            <tr>
              {columns.map((column) => (
                <th
                  ...
                  aria-sort={
                    sort?.header === column.header ? sort.direction : "none"
                  }
                >
                  <button
                    type="button"
      ...
      {!rows.length ? (
        <p className={styles.help} role="status">
          No rows returned. This query completed successfully.
        </p>
      ) : (
        <div className={styles.pagination}>
          <p role="status">
            ...
            {rows.length}; page {currentPage + 1} of {pageCount}
          </p>
