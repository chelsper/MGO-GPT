...
export default function ListColumnEditor({
  ...
  const AdvancedOptions = compact ? "details" : "div";
  ...
  return (
    <details
      className={compact ? listStyles.columns : styles.card}
      onKeyDown={(event) => {
        if (event.key === "Escape") {
    ...
    >
      <summary>{compact ? "Columns" : "Display columns"}</summary>
      <div className={compact ? listStyles.columnOptions : undefined}>
        <p className={styles.muted}>
          {compact
            ? "Customize this view for your visit. Shared defaults are managed in report settings."
            : "Show, rename, format, and reorder returned columns. Hiding a column is not an access restriction. These changes do not run NXT."}
        </p>
        {compact && (
          <div className={listStyles.quickColumns}>
        ...
        <AdvancedOptions
          className={compact ? listStyles.advancedColumns : undefined}
        >
          {compact && <summary>Rename, format &amp; reorder</summary>}
          <div className={styles.stack}>
            {columns.map((column, index) => (
              <div className={styles.toolbar} key={column.header}>
                {compact ? (
                  <strong className={listStyles.columnName}>
                    {column.header}
                  </strong>
                ) : (
            ...
            {!columns.length && (
              <p>
                Refresh the saved list once, then load its returned columns
                here. Or enter an exact output header below.
              </p>
            )}
            {allowAdd && (
