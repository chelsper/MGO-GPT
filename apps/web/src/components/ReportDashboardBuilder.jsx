...
import { DASHBOARD_LIMITS, QUERY_RESULTS_LIMITS, isValidDashboardTableData, validateDashboardQueryId } from "@/app/api/utils/dashboardConfiguration";
...
function newQueryPanel() {
  return { key: newKey("panel"), title: "Query results", layout: "query_results", width: "full", queryId: "", refreshPolicy: "refreshable", columnSettings: [], rows: [], columns: [], values: [] };
}
...
function QueryTableEditor({ panel, onChange, disabled }) {
  ...
  const preview = previewResult?.queryId === panel.queryId ? previewResult : null;
  const queryError = validateDashboardQueryId(queryId);
  const columnSettings = panel.columnSettings || [];
  const headers = [...new Set([...(preview?.headers || []), ...columnSettings.map((column) => column.header)])];
  ...
  function changeQueryId(nextId) {
    ...
    if (columnSettings.length && !window.confirm("Change query ID and reset its column display settings? The previous preview will also be cleared. This changes only the draft until you save.")) return;
    request.current?.abort();
  ...
  function changeColumn(header, patch) {
    ...
    if (!existing && columnSettings.length >= QUERY_RESULTS_LIMITS.columns) return;
    const next = { header, label: "", format: "text", ...existing, ...patch };
    onChange({ columnSettings: existing ? columnSettings.map((column) => column.header === header ? next : column) : [...columnSettings, next] });
  }
  ...
  async function loadPreview() {
    ...
    setError("");
    try {
      const response = await fetch("/api/reports/dashboards/test-query-results", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ queryId }),
        signal: controller.signal,
      ...
      if (!response.ok) {
        ...
        const detail = [400, 401, 403, 413, 502].includes(response.status) && typeof result?.error === "string" ? result.error.trim().slice(0, 500) : "";
        if (detail) {
          setError(detail);
          return;
        }
      ...
      if (String(result?.queryId) !== queryId || result?.dataSource !== "query-results-csv-v1" || !isValidDashboardTableData(result)) {
        throw new Error("invalid_query_preview");
      }
      // Returned cells stay in local state, never in the draft or dashboard snapshot.
      setPreview({ queryId: panel.queryId, headers: result.headers, rows: result.rows });
    } catch {
      if (!controller.signal.aborted) setError("Could not load this query preview. Check its ID, access and result limits, then try again.");
    } finally {
  ...
  return (
    <div className={styles.queryEditor}>
      <p className={styles.warning}>Shared reports expose all returned query columns, including donor information, to the selected viewers. Review the query output and report access before sharing. Display labels and formats do not hide columns.</p>
      <div className={styles.panelFields}>
        <div className={styles.field}>
          <label htmlFor={queryInputId}>Query ID</label>
          <input id={queryInputId} inputMode="numeric" maxLength={16} value={panel.queryId ?? ""} placeholder="30971" disabled={disabled} aria-invalid={Boolean(queryError)} aria-describedby={`${queryInputId}-help`} onChange={(event) => changeQueryId(event.target.value)} />
          <span id={`${queryInputId}-help`} className={styles.help}>{queryError || "Saved NXT query system record ID."}</span>
        </div>
        <label className={styles.field}>
          Refresh policy
          <select value={panel.refreshPolicy || "refreshable"} disabled={disabled} onChange={(event) => onChange({ refreshPolicy: event.target.value })}>
            <option value="refreshable">Refreshable</option><option value="frozen">Frozen</option>
          </select>
        </label>
      </div>
      <p className={styles.help}>Rows and columns come from the query automatically, with no manual axes. Limits: {QUERY_RESULTS_LIMITS.rows.toLocaleString("en-US")} rows, {QUERY_RESULTS_LIMITS.columns} columns, {QUERY_RESULTS_LIMITS.bytes.toLocaleString("en-US")} CSV bytes and {QUERY_RESULTS_LIMITS.cellCharacters.toLocaleString("en-US")} characters per cell. Only Load query preview runs the query here; preview data is not saved and does not refresh or freeze the dashboard.</p>
      <div className={styles.testActions}>
        <button type="button" className={styles.button} disabled={disabled || Boolean(queryError) || pending} onClick={loadPreview}>{pending ? "Loading query preview..." : "Load query preview"}</button>
        {pending ? <span role="status">Loading query {queryId}...</span> : null}
        {error ? <span role="alert" className={styles.error}>{error}{preview ? " Showing the last successful preview." : ""}</span> : null}
      </div>
      {headers.length ? (
        <details className={styles.columnSettings}>
          <summary>Column display settings (optional)</summary>
          <p className={styles.help}>Settings match exact query headers. Text preserves NXT amounts as returned. Choose Number or Currency (USD) explicitly to format amounts; unrecognized values remain text.</p>
          {headers.map((header) => {
            const setting = columnSettings.find((column) => column.header === header);
            const atLimit = !setting && columnSettings.length >= QUERY_RESULTS_LIMITS.columns;
            return (
              <fieldset key={header} className={styles.cellEditor} disabled={disabled || atLimit}>
                <legend>{header}</legend>
                {!preview?.headers.includes(header) ? <p className={styles.help}>This saved header has not been confirmed in the current preview.</p> : null}
                <div className={styles.columnFields}>
                  <label className={styles.field}>Display label for {header}<input maxLength={120} value={setting?.label || ""} placeholder={header} onChange={(event) => changeColumn(header, { label: event.target.value })} /></label>
                  <label className={styles.field}>Format for {header}<select value={setting?.format || "text"} onChange={(event) => changeColumn(header, { format: event.target.value })}><option value="text">Text (as returned)</option><option value="number">Number</option><option value="currency">Currency (USD)</option></select></label>
                  <button type="button" className={styles.button} disabled={disabled || !setting} aria-label={`Reset display settings for ${header}`} onClick={() => onChange({ columnSettings: columnSettings.filter((column) => column.header !== header) })}>Reset display</button>
                </div>
              </fieldset>
            );
          })}
        </details>
      ) : <p className={styles.help}>Load a preview to see the returned columns and customize their display.</p>}
      {preview ? (
        <>
          <p className={styles.success} role="status">Query preview only. Not saved to the dashboard.</p>
          <QueryResultsTable title={`${panel.title || "Query results"} preview`} headers={preview.headers} rows={preview.rows} columnSettings={columnSettings} disabled={disabled} />
        </>
      ) : null}
    </div>
  );
...
export default function ReportDashboardBuilder({ value, onChange, disabled = false }) {
  ...
  const tableCount = panels.filter((panel) => panel.layout === "query_results").length;
  const queryCount = allValues.filter((cell) => cell.source === "query_count").length + tableCount;
  const canAddQueryTable = panels.length < DASHBOARD_LIMITS.panels && tableCount < QUERY_RESULTS_LIMITS.panels && queryCount < DASHBOARD_LIMITS.queries;
  ...
  function removePanel(panel) {
    if (disabled) return;
    const confirmed = panel.layout === "query_results"
      ? window.confirm(`Remove query table "${panel.title}"? This changes only the draft until you save.`)
  ...
  function changeLayout(panel, layout) {
    if (panel.layout === "query_results" || !["rows", "table", "metric"].includes(layout)) return;
  ...
  return (
    <div className={styles.builder}>
      <div className={styles.toolbar}>
        ...
        >Add panel</button>
        <button type="button" className={styles.button} disabled={disabled || !canAddQueryTable} onClick={() => { if (canAddQueryTable) changePanels([...panels, newQueryPanel()]); }}>Add query table</button>
        </div>
      </div>
      <p className={styles.help}>{panels.length}/{DASHBOARD_LIMITS.panels} panels; {allValues.length}/{DASHBOARD_LIMITS.values} values; {queryCount}/{DASHBOARD_LIMITS.queries} saved-query sources; {tableCount}/{QUERY_RESULTS_LIMITS.panels} query tables. Each query table uses one saved-query source. Leave a static value blank for unknown. Zero is a valid value.</p>
      {!panels.length ? <p className={styles.empty}>Add a panel for numeric rows, a table, or metrics, or add a query table for actual query results.</p> : null}
      {panels.map((panel, panelIndex) => (
        <section key={panel.key} className={styles.editorPanel} aria-label={`Edit panel ${panelIndex + 1}`}>
          ...
          <div className={styles.panelFields}>
            ...
            {panel.layout === "query_results" ? <div className={styles.field}>Layout<span className={styles.help}>Query results table</span></div> : <label className={styles.field}>
              Layout
          ...
          {panel.layout === "query_results" ? <QueryTableEditor panel={panel} disabled={disabled} onChange={(patch) => updatePanel(panel, patch)} /> : (
          <>
